...
> [!IMPORTANT]
> [Bugzilla 51940 - 分離プロセスやアプリケーションのカスタム クラスとサービスが適切にオーバー ロードを解決するのには失敗](https://bugzilla.xamarin.com/show_bug.cgi?id=51940)Xamarin.Android サービスが正しく起動はしないレポートと、`IsolatedProcess`に設定されている`true`します。 このガイドは、参照用に提供されます。 Xamarin.Android アプリケーションは、Java で記述されたプロセス外のサービスと通信できる必要があります。
