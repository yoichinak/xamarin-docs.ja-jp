...
* **サービスが開始** &ndash; A_サービスを開始しました_(アクティビティ) などの他の Android コンポーネントによって開始され、は何かに明示的に指示するまで、バック グラウンドで継続してを実行するサービスは、サービスが停止します。 バインドされているサービスとは異なり、開始されるサービスに直接バインドされたすべてのクライアントはありません。 このため、必要に応じて、正常に再起動する可能性がありますので、開始されているサービスを設計する重要なは。
...
既存の Xamarin.Android アプリケーションは、Android 8.0 で発生する可能性のある問題を回避するためにバック グラウンド作業を実行する方法を変更する必要があります。 Android のサービスをいくつかの実用的な alterantives を次に示します。
