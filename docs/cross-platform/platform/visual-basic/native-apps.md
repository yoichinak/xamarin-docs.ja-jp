...
IOS アプリケーションでは、 `TodoItemManager` とが、 `XmlStorageImplementation` 次のコードスニペットに示すように **AppDelegate.cs** ファイルに作成されます。 最初の4行は、データが格納されるファイルへのパスを作成するだけです。最後の2行は、インスタンス化されている2つのクラスを示しています。
