...
> [!NOTE]
> **注**: は各プラットフォームのプロジェクトで効果を提供する省略可能です。 いずれかが登録されていない場合に効果を使用しようとしています。 には、何も null 以外の値が返されます。
...
> [!NOTE]
> **注**: 効果インスタンスは、1 つのコントロールにのみアタッチできます。 このため、2 つのコントロールで使用するには、2 回、特殊効果を解決する必要があります。
