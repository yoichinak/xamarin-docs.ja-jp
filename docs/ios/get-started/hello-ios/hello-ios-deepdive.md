...
このシーンによって表されるコンテンツビュー階層のカスタム ビュー コントローラー クラスは、**プロパティ パッド**の **[ID]** セクションで **[Class]** プロパティを編集することによって設定できます。 たとえば、私たちの **Phoneword** アプリケーションでは、下のスクリーンショットに示すように、最初の画面のビュー コントローラーとして `ViewController` を設定します。
