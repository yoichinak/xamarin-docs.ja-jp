...
6. **Solution Pad** で **Main.storyboard** ファイルをダブルクリックして開きます。 必ず Visual Studio iOS Designer を使用してファイルを開いてください。(ストーリーボードを右クリックし、 **[ファイルを開くアプリケーションの選択]、[iOS Designer]** の順に選択します)。 これで目視で確認しながら UI を作成できます。
