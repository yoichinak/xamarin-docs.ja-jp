...
Objective-C コードと通信するための低レベルのランタイムが [MonoTouch.ObjCRuntime](#objcruntime) です。 この上にさらに、[Foundation](#foundation)、CoreFoundation、[UIKit](#uikit) 向けのバインディングが与えられます。
...
- API の IDE 内でいろいろ試すことを奨励する
  ...
  - たとえば、次のような弱く型指定された配列を公開する代わりに
    ...
    これにより Visual Studio for Mac で、API を閲覧中に、オートコンプリート機能が利用できます。また、戻り値であらゆる `System.Array` 操作が可能になり、LINQ に加わることが戻り値に許可されます。
...
Objective-C クラスでは、このプログラミング パターンが使用されるクラスでプロパティが公開されることがわかります。そのプロパティは通常、`delegate` という名称ですが、インターフェイスの必須およびオプション部分を実装するために必要です。
