...
まず、Objective-C を C# に公開する方法が必要です。これは、セレクターを介して行われます。セレクターは、オブジェクトまたはクラスに送信されるメッセージです。 Objective-C では、 これは [objc_msgSend](~/cross-platform/macios/binding/overview.md) 関数を使用して実行します。
セレクターの使用方法の詳細については、[Objective-C セレクター](~/ios/internals/objective-c-selectors.md)ガイドを参照してください。 また、マネージコードを Objective-C に公開する方法も必要です。これは、Objective-C はマネージコードについて何も知らないことが原因で、より複雑になります。 この問題を回避するには、*レジストラー*を使用します。 これらの詳細については、次のセクションで詳しく説明します。

## <a name="registrars"></a>レジストラー

前述のように、レジストラーはマネージコードを Objective-C に公開するコードです。 これを行うには、NSObject から派生したすべてのマネージクラスのリストを作成します。
...
Xamarin の [`btouch`](https://github.com/xamarin/xamarin-macios/blob/master/src/btouch.cs)と呼ばれるジェネレーターは、これらの定義ファイルを受け取り、.net ツールを使用して[それらを一時アセンブリにコンパイル](https://github.com/xamarin/xamarin-macios/blob/master/src/btouch.cs#L318) します。 ただし、この一時アセンブリは、Objective-C コードを呼び出すことができません。 その後、ジェネレーターは一時アセンブリを読み取り C# 、実行時に使用できるコードを生成します。
