...
新しい Xamarin.iOS プロジェクトでは、**デバッグ**および**リリース**の "_ビルド構成_" が自動的に設定されます。 **リリース** ビルドを正しく構成するには、次の手順に従います。
...
5. リンクを設定し、未使用のコードを削除することで、アプリケーションの全体のサイズが小さくなります。 ほとんどの場合、 **[リンカーの動作]** は既定値の **[フレームワーク SDK のみをリンクする]** に設定する必要があります。 **[リンクしない]** オプションを使用すると、 **[フレームワーク SDK のみをリンクする]** オプションでリンクから除外されるパブリックでない iOS API が Xamarin.iOS に存在するため、Apple がアプリを拒否する可能性があります。 **[すべてをリンクする]** により、サードパーティのライブラリを含む、プロジェクト内のすべてのアセンブリからコードが削除されるため、注意して使用する必要があります。また、使用されているライブラリ コードが静的コード分析によって決定されるため、サードパーティのライブラリがリンカーで検出できないリフレクションによってのみ使用できるコードが除外される場合があります。 コードの欠落による実行時のエラーを回避するには一部のクラスやメソッドを手動で保持する必要があるため、 **[すべてをリンクする]** は注意して使用してください。 詳細については、「[Xamarin.iOS アプリをリンクする](~/ios/deploy-test/linker.md)」ガイドを参照してください。
