...
アプリ ID を定義するときに必要なアプリケーションサービスを選択して構成するだけでなく、`Info.plist` ファイルと `Entitlements.plist` ファイルの両方を編集して、tvOS プロジェクトのアプリ ID と権利を構成する必要もあります。
...
Apple TV App Store に含まれるように Apple によって tvOS アプリが受け入れられるようにするには、実行されるすべての Apple TV デバイスに対して、適切なアイコン、起動、および上部の棚イメージが必要です。 ITunes Connect にアップロードされる前に、`Assets.car` ファイルにコンパイルされ、tvOS アプリのバンドルに含まれる必要なイメージアセットを追加する必要があります。
...
5. Apple では、tvOS アプリで bitcode を使用する必要もあります。 **リリース**構成の下で、 **[追加の mtouch 引数]** ボックスに `--bitcode=asmonly` を追加します。
...
`.ipa`を保存すると、Visual Studio for Mac のアーカイブワークフローによってアプリケーションローダーが自動的に開きます。
