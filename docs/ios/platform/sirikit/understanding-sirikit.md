...
description: このドキュメントでは、Xamarin.iOS アプリで SiriKit を操作するために必要な主要な概念について説明します。 たとえば、インテントについても説明とインテント UI 拡張機能、SiriKit アクセス許可、優れたエクスペリエンスでは、その他の設計。
ms.prod: xamarin
