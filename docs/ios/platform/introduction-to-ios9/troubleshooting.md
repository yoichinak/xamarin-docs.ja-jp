...
IOS 8 (およびそれ以前) では、ストーリーボードの UI 要素は、**右**  &  **左**の属性 ( `NSLayoutAttributeRight`  &  `NSLayoutAttributeLeft` ) と**先頭**の  &  **末尾**の属性 () の組み合わせを `NSLayoutAttributeLeading`  &  `NSLayoutAttributeTrailing` 同じレイアウトで使用できます。
