...
既定のアクションでは、何も実行されません。 上記のサンプルでは、これは、目標 C ランタイムアンワインドマネージフレームを可能にすることを意味します。 これは、目的の C ランタイムがマネージフレームをアンワインドする方法を認識しないため、問題になります。たとえば、そのフレーム内で`catch` or `finally`句を実行することはありません。
...
フレーム0-3 はネイティブフレームであり、アンワインダーランタイムの stack のスタックは、これらのフレームをアンワインド_することができ_ます。 特に、すべての目的 C `@catch`または`@finally`句が実行されます。
...
これは、アンワインダー stack がマネージ`catch`句を認識しておらず、 `finally`句が実行されないためです。

上記のコードサンプル_が_有効になっている場合、これは、目的 c の例外 (xamarin. iOS と xamarin. Mac [`NSSetUncaughtExceptionHandler`][2]が使用されている) を通知するメソッドが、その時点で、任意の c 例外を変換しようとしているためです。マネージ例外にします。
...
3. この目的では、C ランタイムは`@catch`ハンドラーを検出`NSGetUncaughtExceptionHandler`せず、を呼び出し、xamarin. iOS/xamarin. Mac によってインストールされたハンドラーを呼び出します。
...
ここでは、唯一のマネージフレームはフレーム8-10 ですが、マネージ例外はフレーム0でスローされます。 これは、mono ランタイムがネイティブフレーム0-7 をアンワインドする必要があることを意味します。これにより、前に説明した問題と同等の問題が発生します`@catch` 。 `@finally`ただし、mono ランタイムはネイティブフレームをアンワインドしますが、目的の C または句は実行されません.
...
また、このフレームをアンワインドする Mono ランタイムが認識していないため、句は実行されません。`@finally`

このようなバリエーションとして、マネージコードでマネージ例外をスローした後、ネイティブフレームをアンワインドし`catch`て最初のマネージ句を取得します。
...
マネージ`UIApplication:Main`メソッドは、ネイティブ`UIApplicationMain`メソッドを呼び出します。次に、iOS はネイティブコードを実行してから、最終的に`AppDelegate:FinishedLaunching`マネージメソッドを呼び出します。マネージ例外が発生した場合、スタックには、まだ多数のネイティブフレームがあります。破棄
...
フレーム0-1 と27-30 は管理されていますが、間にあるものはすべてネイティブです。 これらのフレームによって Mono がアンワインド`@catch`さ`@finally`れた場合、目標 C または句は実行されません。

### <a name="scenario-2---not-able-to-catch-objective-c-exceptions"></a>シナリオ 2-目的 C の例外をキャッチできない

次のシナリオでは、目的 c の例外が別の方法で処理さ`catch`れたため、マネージハンドラーを使用して目的の c の例外をキャッチすることはでき_ません_。

1. 目的 C の例外がスローされます。
2. 目的の C ランタイムはスタックにステップインし (ただし、アンワインドしません) `@catch` 、例外を処理できるネイティブハンドラーを探します。
3. 目的の C ランタイムは、ハンドラー `@catch`を検索し、スタックをアンワインドし`@catch`て、ハンドラーの実行を開始します。
...
これは、xamarin より前の macOS バージョンで Xamarin. Mac アプリをデバッグする場合にも非常に一般的です。 Mac でサポートされているのは、デバッガー内のほとんどの UI オブジェクトを検査すると、実行中のプラットフォームに存在しないセレクターに対応するプロパティを取得しようとするためです (Xamarin. Mac では、より高い macOS バージョンがサポートされています)。 このようなセレクターを呼び`NSInvalidArgumentException`出すと、("認識されないセレクターがに送信されました") がスローされ、最終的にプロセスがクラッシュします。
...
例外がインターセプト`Runtime.MarshalManagedException`されると、と`Runtime.MarshalObjectiveCException`の2つの新しいイベントが発生します。

両方のイベントには`EventArgs` 、スローされた元の例外を含むオブジェクト`Exception` ( `ExceptionMode`プロパティ) と、例外をマーシャリングする方法を定義するプロパティが渡されます。

イベントハンドラーでプロパティを変更して、ハンドラーで実行されるカスタム処理に従って動作を変更できます。 `ExceptionMode` 1つの例として、特定の例外が発生した場合にプロセスを中止することがあります。

プロパティの`ExceptionMode`変更は1つのイベントに適用されますが、今後受け取る例外には影響しません。
...
を除き`ExceptionMode` `MarshalManagedException` `MarshalObjectiveCException` 、これらの値はイベントおよびイベントに渡される値と同じです。 `disable`

この`disable`オプションは、_ほとんど_の場合、インターセプトを無効にします。ただし、実行のオーバーヘッドが追加されない場合でも例外を受け取ることになります。 これらの例外に対してマーシャリングイベントが発生しても、既定のモードは実行中のプラットフォームの既定のモードになります。
