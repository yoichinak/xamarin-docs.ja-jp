...
プロジェクトの変更がコード リポジトリを共有する他の開発者がチェックインされるたびに通常に継続的インテグレーション (CI) 継続的な共通コード ベースへのすべての開発者の変更をマージすることによってこのような複雑さをしないでください。 各チェックインの自動ビルドをトリガーし、新しく導入されたコードが既存のコードを中断していないことを確認する自動テストを実行します。  この方法では、CI は、エラーおよび問題を直ちに表示され、作業の最新のそれぞれ他のユーザーとすべてのチーム メンバーが保持を実現します。 これは、結果、結束力と安定したコードベースします。
...
-  **継続的インテグレーションサーバー** – の継続的インテグレーションサーバーはすべて、プロジェクトの成果物 (ソース コード、画像、ビデオ、データベース、自動テストなど) の収集、アプリをコンパイルおよび自動テストの実行を担当します。 もう一度は多くのオープン ソースおよび商用の CI サーバー ツールです。
...
ここでも、継続的インテグレーションは、変更のコミットの機能により、CI サーバー プロジェクトをビルドし、ソース コードが正しいことを確認する自動テストを実行します。 CI サーバーが、担当開発者に通知ビルド エラーまたはエラーのテストがある場合は、(電子メール、IM、Twitter、Growl など)、問題を修正できるようにします。 (CI のサーバーも拒否できますコミット場合は、「ゲート チェックイン」と呼ばれるエラーが発生します。)
